...
Create `v0.0.5` only after those checks are complete. Do not move or reuse a published
tag.
...
```bash
gh attestation verify jobstreaming-0.0.5-py3-none-any.whl \
  --repo ebarti/jobstreaming
gh attestation verify jobstreaming-0.0.5.tar.gz \
  --repo ebarti/jobstreaming
```
...
/tmp/jobstreaming-release-check/bin/python -m pip install \
  jobstreaming-0.0.5-py3-none-any.whl
/tmp/jobstreaming-release-check/bin/python -m pip check
/tmp/jobstreaming-release-check/bin/python -c \
  'from importlib.metadata import version; assert version("jobstreaming") == "0.0.5"'
```
