...
from jobstream.model import CompensationInterval, JobType, Site

_JOBSTREAM_LOG_LEVEL = logging.INFO


def create_logger(name: str):
    logger = logging.getLogger(f"JobStream:{name}")
    logger.propagate = False
    if not logger.handlers:
        logger.setLevel(_JOBSTREAM_LOG_LEVEL)
...
def set_logger_level(verbose: int):
    ...
    """
    global _JOBSTREAM_LOG_LEVEL
    if verbose is None:
        return
    levels = {2: "INFO", 1: "WARNING", 0: "ERROR"}
    if verbose not in levels:
        raise ValueError("verbose must be 0, 1, or 2")
    _JOBSTREAM_LOG_LEVEL = getattr(logging, levels[verbose])
    for logger_name in logging.root.manager.loggerDict:
        if logger_name.startswith("JobStream:"):
            logging.getLogger(logger_name).setLevel(_JOBSTREAM_LOG_LEVEL)
